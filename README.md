...
LuKK 的小红书长文卡片排版工具。把已经写好的 Markdown 长文贴进去，自动分页、套主题、微调排版，然后批量导出 3:4 PNG 卡片。

这个工具来自 LuKK（困困）的内容生产流程：先把内容写清楚，再把它快速变成一组能发布、能连续阅读的小红书图文卡片。
...
- **智能分页**：正文过长时自动拆成多张卡片。
- **Markdown 支持**：支持一级标题、引用、`**加粗**`、`==高亮==` 和空行分段。
- **多套主题**：浅底、暖色、冷色、深色主题都可切换。
- **排版微调**：标题字号、正文字号、行距、标题字体和高亮样式都可调整。
...
这是一个偏个人工作流的开源小工具。默认署名保留了“困困”，你可以在界面里改成自己的账号名、栏目名，也可以直接留空。
